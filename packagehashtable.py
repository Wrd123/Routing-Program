class PackageIDHashTable:
    def __init__(self, size=50):
        self.size = size
        self.table = [[] for _ in range(self.size)]  # Initialize hash table

    def _hash_function(self, key):
        return key % self.size  # Simple modulo hash function

    def insert(self, package_id, address, city, state, zip_code, deadline, mass, special_notes="", status="At Hub", delivery_time=None):
        ...
        # Check if package ID already exists and update if necessary
        for item in self.table[index]:
            if item["Package ID"] == package_id:
                item.update(package_data)
                return
...
if __name__ == '__main__':
    ...
    for pkg in packages:
        hash_table.insert(*pkg)

    hash_table.display()
    print("\nSearching for package ID 3:")
